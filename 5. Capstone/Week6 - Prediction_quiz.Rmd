...
logpred <- predict(model.AIC, ames_test)
pred <- exp(logpred)
mean(pred)
```
...
# Extract Predictions TRAINING SET
tpredict.full <- exp(predict(model.full, ames_train))

# Extract Residuals
tresid.full <- ames_train$price - tpredict.full

# Calculate RMSE
trmse.full <- sqrt(mean(tresid.full^2))
trmse.full

# Extract Predictions TRAINING SET
tpredict.AIC <- exp(predict(model.AIC, ames_train))

# Extract Residuals
tresid.AIC <- ames_train$price - tpredict.AIC

# Calculate RMSE
trmse.AIC <- sqrt(mean(tresid.AIC^2))
trmse.AIC
```
...
HPM.out = as.data.frame(cbind(exp(confint(pred.test.HPM)),
                          price = ames_test$price))

# Fix names in dataset
colnames(HPM.out)[1:2] <- c("lwr", "upr")  #fix names

# Get Coverage
pred.test.HPM.coverage <- HPM.out %>% summarize(cover = sum(price >= lwr & price <= upr)/n())
...
# Get dataset of predictions and confidence intervals
MPM.out = as.data.frame(cbind(exp(confint(pred.test.MPM)),
                          price = ames_test$price))

# Fix names in dataset
colnames(MPM.out)[1:2] <- c("lwr", "upr")  #fix names

# Get Coverage
pred.test.MPM.coverage <- MPM.out %>% summarize(cover = sum(price >= lwr & price <= upr)/n())
