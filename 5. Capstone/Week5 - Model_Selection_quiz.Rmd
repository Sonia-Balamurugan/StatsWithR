...
model1 <- lm(log(price) ~ log(area) + log(Lot.Area) + Bedroom.AbvGr + Overall.Qual + Land.Slope, data = ames_train)
stepAIC(model1, direction = "backward")

model2 <- bas.lm(log(price) ~ log(area) + log(Lot.Area) + Bedroom.AbvGr + Overall.Qual + Land.Slope, data = ames_train, prior = "BIC")
summary(model2)
```
...
m3 = lm(log(price) ~ Bedroom.AbvGr, data = ames_train)
summary(m3)

m4 = lm(log(price) ~ log(area) + Bedroom.AbvGr, data = ames_train)
summary(m4)
```
...
m5 = lm(log(price) ~ log(area), data = ames_train)
neighbr <- ames_train %>% select(price, area, Neighborhood) %>% mutate(Resid = m5$residuals)
neighbr %>% group_by(Neighborhood) %>% summarise(mean = mean(Resid))
summary(m5)
```
...
Sqd <- neighbr %>% group_by(Neighborhood) %>% summarise(sq = mean(Resid^2))
```
...
m6 <- lm(log(price) ~ Overall.Qual + Bsmt.Qual + Garage.Qual, data = ames_train)
summary(m6)
```
...
m7 <- lm(log(price) ~ Overall.Cond + Overall.Qual, data = ames_train)
scprice <- ames_train %>% select(price, Overall.Cond, Overall.Qual, MS.SubClass) %>% mutate(Predicted = m7$fitted)
scprice %>% group_by(MS.SubClass) %>% summarise(median = median(Predicted))
...
infl <- lm.influence(m7)
which.max(infl$hat)
which.max(infl$sigma)
which.max(infl$wt.res)
```
...
norm <- ames_train %>% subset(Sale.Condition == "Normal")
```
...
m8 = lm(log(price) ~ log(area), data = norm)
summary(m8)
```
